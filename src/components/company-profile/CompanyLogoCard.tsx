...
// frontend/src/components/company-profile/CompanyLogoCard.tsx
import { useModal } from "../../hooks/useModal";
import { Modal } from "../ui/modal";
import Button from "../ui/button/Button";
import { useEffect, useState, useRef } from "react";
...
export default function CompanyLogoCard() {
  const { isOpen, openModal, closeModal } = useModal();
  const { companyData, loading, updateLogo } = useCompanyData();
  const [selectedLogo, setSelectedLogo] = useState<string | null>(null);
  const [isSaving, setIsSaving] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    if (companyData?.logotipo64) {
      setSelectedLogo(companyData.logotipo64);
    }
  }, [companyData]);
