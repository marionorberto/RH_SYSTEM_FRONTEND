// src/components/Discounts/DiscountTable.tsx
import {
  Table,
  TableBody,
  TableCell,
...
export default function DiscountTable({
  ...
  const getDiscountTypeBadge = (type: string) => {
    ...
    } else if (type.includes("Segurança Social")) {
  ...
  const getTotalValue = (discount: Discount) => {
    if (discount.tipoCalculo === "Percentual") {
      return `${discount.value}%`;
    }
    return formatCurrency(discount.value * discount.unities);
  };
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-8 dark:border-gray-800 dark:bg-white/[0.03]">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">
            Tente ajustar os filtros ou adicionar um novo desconto.
          </p>
        </div>
      </div>
    );
              ...
              >
                <TableCell className="py-3">
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 overflow-hidden rounded-full bg-gradient-to-br from-red-500 to-red-600 flex items-center justify-center">
                      <span className="text-white font-medium text-sm uppercase">
                        {discount.employee.employee_name.charAt(0)}
                      </span>
                    </div>
                    <div>
                      <p className="font-medium text-gray-800 text-theme-sm dark:text-white/90">
                        {discount.employee.employee_name}
                      </p>
                    </div>
                  </div>
                </TableCell>
                <TableCell className="py-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {discount.discountTypeRel?.typeName ||
                ...
                <TableCell className="py-3">
                  <p className="font-semibold text-gray-800 dark:text-white/90">
                    {discount.tipoCalculo === "Percentual"
                      ? `${discount.value}%`
                      : formatCurrency(discount.value)}
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    Total: {getTotalValue(discount)}
                  </p>
                </TableCell>
                <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400">
                  <p>Unidades: {discount.unities}</p>
                  <p className="text-xs">Dias: {discount.days}</p>
                </TableCell>
                <TableCell className="py-3">
                  {getProcessingTypeBadge(discount.processingType)}
                </TableCell>
                <TableCell className="py-3">
                  {getCalculoTypeBadge(discount.tipoCalculo)}
                </TableCell>
                <TableCell className="py-3 text-gray-500 text-theme-sm dark:text-gray-400">
